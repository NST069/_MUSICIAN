...
public class JKnob
    ...
    /**
     * Paint the WAVEGEN.JKnob on the graphics context given.  The knob
     ...
     *
     * @return the preferred size of the WAVEGEN.JKnob.
     */
     ...
     *
     * @return the minimum size of the WAVEGEN.JKnob.
     */
    ...
    public void mouseDragged(MouseEvent e) {
        if (pressedOnSpot) {
            ...
            double newtheta = Math.atan2(mxp, myp);
            if(bounds!=null && (Math.abs(newtheta)>bounds.rightBound)){
                return;
            }
            else {
                theta=newtheta;
    ...
    public void setStopper(double bound){
        bounds = new Bounds(bound);
    }

    /**
     * Here main is used simply as a test method.  If this file
     * is executed "java WAVEGEN.JKnob" then this main() method will be
     * run.  However, if another file uses a WAVEGEN.JKnob as a component
     * and that file is run then this main is ignored.
     */
    public static void main(String[] args) {

        JFrame myFrame = new JFrame("WAVEGEN.JKnob Test method");

        Container thePane = myFrame.getContentPane();

        // Add a WAVEGEN.JKnob to the pane.
